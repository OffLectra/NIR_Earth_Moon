...
Kep_param_vec AGESK_to_KE(ASK_param_vec v_AGESK) {
    ...
    int ToO = 0;
//    v_KE.DateTime = v_AGESK.DateTime;
...
//          }
    ...
    //__p____________________________________________________________________
          //       v_KE.p = c*c/mu;
          switch (ToO) {
          case 0:{
                 v_KE.p = v_KE.a*(1-pow(v_KE.e,2));
    ...
    //__i____________________________________________________________________
          v_KE.i  =  acos(c3/c);
          v_KE.i  =  check_inc_rad(v_KE.i);
          if ( fabs(v_KE.i) < bm) {
              v_KE.i = bm * copysign(1.0, v_KE.i);// с нулем все просто, делаем его б.м.(bm)
          }
    //__RAAN__________________________________________________________________
          v_KE.RAAN  =  atan2(c1/c/sin(v_KE.i), (-c2)/c/sin(v_KE.i));
          //v_KE.RAAN  =  atan2(c1/sqrt(c1*c1+c2*c2), (-c2)/sqrt(c1*c1+c2*c2));
          //v_KE.RAAN  =  atan2(c1/sqrt(sqr(c1)+sqr(c2)),(-c2)/sqrt(sqr(c1)+sqr(c2)));
          v_KE.RAAN = check_RAAN_rad(v_KE.RAAN);

    //__omega_________________________________________________________________
//          v_KE.om  =  atan2(f3/(lap*sin(v_KE.i)),(c1*f2-c2*f1)/(c*lap*sin(v_KE.i)));
          v_KE.om  =  atan2(f3/(lap*sin(v_KE.i)),(cos(v_KE.RAAN)*f1+sin(v_KE.RAAN)*f2)/(mu*v_KE.e));
          v_KE.om  =  check_omega_rad(v_KE.om);
    //__u____________________________________________________________________
//          v_KE.u  =  atan2(v_AGESK.z/(v_KE.r *sin(v_KE.i)),
//          (v_AGESK.y/v_KE.r - cos(v_KE.RAAN) * cos(v_KE.i) * v_AGESK.z/(v_KE.r*sin(v_KE.i)))/(sin(v_KE.RAAN))); // странная формула
          v_KE.u  =  atan2(v_AGESK.z/(v_KE.r *sin(v_KE.i)),(v_AGESK.x*cos(v_KE.RAAN)+v_AGESK.y*sin(v_KE.RAAN))/v_KE.r);
